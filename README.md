...
| 成績の保持件数の管理が、問題集ごとではなくテーブル全体を対象にしていた | **他の問題集を10回解くと、その問題集の成績グラフが空になる** |
| ホーム画面のボタンは10個固定だが、問題集がないボタンの後始末をしていなかった（該当処理がコメントアウトされたままだった） | レイアウトの仮の文字（"22" など）が残る。ページが使い回されると**別のタブのタイトルとリスナーが残り、無関係な問題集が開く** |
...
- `StatisticsDbHelperTest` — 成績の保存・取り出し・並び順・問題集ごとの保持件数を検証
